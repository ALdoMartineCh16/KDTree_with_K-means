...
{
    KDTree kdtree(2);
    kdtree = readAndConvert_("Resources/data2k.csv");
    Direct direct;
    direct = readAndConvertDirect_("Resources/data2k.csv");
    int n;
    vector<CoorD> vecinos;
    int opcion;
    auto start = chrono::high_resolution_clock::now();
    auto end = chrono::high_resolution_clock::now();
    auto result = std::make_pair(std::vector<std::vector<CoorD>>{}, std::vector<CoorD>{});
    ...
    do
    {
        ...
        case 7:
            start = chrono::high_resolution_clock::now();
            result = direct.KMeans(k);
            end = chrono::high_resolution_clock::now();
            clusters = result.first;
            centroids = result.second;
            cout << "Los clusters son:" << std::endl;
